...
export interface components {
    schemas: {
        ...
        LlmCallSummary: {
            ...
            /** @description 模型名称 */
            model: string;
            /** @description 业务对象类型（如 interview_session=面试会话、resume_file=简历文件、 resume_profile=简历画像），弱引用无外键，用于按业务对象归因调用成本 */
            bizType?: string | null;
            /**
             * Format: int64
             * @description 业务对象 ID，允许悬空（业务数据删除后仍保留历史观测记录）
             */
            bizId?: number | null;
            ...
            /** @description 失败原因（落库时已截断至 500 字符） */
            ...
            /** @description 本次请求的输出上限；与 tokenOutput 接近说明输出顶到了上限， 是"JSON 被截断"类失败的直接证据 */
            maxTokens?: number | null;
            /** @description 修复重试序号，0=首次调用 */
            retryAttempt: number;
            /** @description MQ 任务重试轮次，0=首次处理；与"修复重试序号 retryAttempt"不是同一维度： attemptNo 由任务消息的 retryCount 决定，retryAttempt 由同一次任务内的 提示词修复调用序号决定。同步 HTTP 接口发起的调用没有任务重试概念，恒为 0 */
            attemptNo: number;
            /** @description 上游 HTTP 状态码；429 表示限流、5xx 与 3xx 表示上游/环境故障（可重试）、 其余 4xx 表示请求被拒（重发同样请求不会成功） */
            httpStatus?: number | null;
            /** @description 上游停止原因；length 表示输出被 max_tokens 截断，是排查 "JSON 格式无效"这类失败的关键字段 */
