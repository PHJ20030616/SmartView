...
- 每次执行任务（仅仅针对复杂任务，首先进行任务复杂度判定，简单任务直接执行，不走以下规则）需要遵循先制定任务计划，确保执行计划覆盖核心任务（你需要和我沟通确定这个任务的各种细节，也是为了让我为这个项目做整体的 布局规划）执行后，由计划撰写完成我审查确认---确认后，开始按照计划执行---计划执行完成后，调用全新的子Agent去审查你的代码，确保代码没有逻辑和业务上的漏洞
