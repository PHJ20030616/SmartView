...
- 追问候选池（0-2 道）：基于当前回答内容生成的追问题，分补缺口（GAP）与深挖（DEEP）两型
...
- 追问**不可用**（`pickFollowUp` 返回 null：池中无追问候选，或得分 <40 被门控）且 同阶段换题候选为空：使用下一阶段入口候选 `NEXT_STAGE`
...
#### 规则 5：正常流程（兜底）
- 回答质量好（得分 >= 70）且追问可用 且未达深度上限：`FOLLOW_UP`（优先 `DEEP` 型）
- 回答质量中等（40 <= 得分 < 70）且同阶段有换题候选：`SWITCH_TOPIC`
- 回答质量差（得分 < 40）且同阶段有换题候选：`SWITCH_TOPIC`
- 其他情况：`NEXT_STAGE`

> 追问可用性口径见 3.3（`StagePolicyEngine.pickFollowUp` 非 null）。
> 40~70 分的追问只发生在**没有换题候选**的兜底路径（规则 4 之后），不是规则 5 的分支。
...
| 追问候选 | 随 `/evaluate` 响应返回，**与回答评估并行生成**，并在内存中并入本次决策 | 0-2 道 | 基于回答内容的深度追问 |
...
**追问生成与评估解耦（v1.3 起）**：追问候选只依赖题目、候选人回答与期望要点，不依赖评估结论，
因此两者在同一请求内并行执行，用户等待从"评估 + 追问生成"之和收敛为两者中的较大值。
代价是生成侧看不到得分，无法预判追问类型——改为**两型各生成一道**（`followUpKind`：`GAP` 补缺口、
`DEEP` 深挖），得分门控移到决策侧（见 3.3）。
...
**包含（v1.3 起：两型各 1 道，共 0-2 道）：**
- 补缺口型（`followUpKind=GAP`）：针对回答中未展开、含糊或明显缺失的要点追问
- 深挖型（`followUpKind=DEEP`）：针对回答中讲得较清楚或较独特的点继续深挖

**生成规则（AI 服务侧，确定性）：**
- 当前主题追问深度已达 `max_follow_up_depth`：不生成
- 回答为空或明确表示不会（空白 / "不会 / 不熟悉"等关键词）：不生成（保持 0 次 LLM 调用）
- 其余情况：GAP、DEEP 各生成一道（并发），**不按得分过滤**

**采用规则（Spring `StagePolicyEngine.pickFollowUp`，得分门控在决策侧）：**
- 回答得分 < 40：追问候选一律不可用（质量太低，追问无意义），改走换题 / 模板化过渡题
- 回答得分 40-70：优先采用 `GAP`
- 回答得分 ≥ 70：优先采用 `DEEP`
- 优先类型缺失（单条生成失败，或 Redis 旧数据无 `followUpKind`）：回退为首个可用追问候选

> 注意：40~70 分只有在**没有同阶段换题候选**时才会真的用到追问（规则 5 先判换题）；
> 有换题候选就换题，`GAP` 只在兜底路径生效。≥70 分则优先追问（规则 5 第一分支）。

> 得分门控从"生成侧"移到"决策侧"是 v1.3 解耦的结果：生成侧与评估并行，拿不到得分；
> 若把门控留在生成侧，就必须等评估完成才能生成追问，等待时间回到两者之和。
...
- [ ] 追问候选池在回答提交时与评估**并行**生成，只基于回答内容（得分门控在 `StagePolicyEngine`）
