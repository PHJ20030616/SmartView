...
简历解析状态。RabbitMQ 连接默认使用 `localhost:5672`、用户 `smartview`、虚拟主机
`smartview`，可通过 `RABBITMQ_HOST`、`RABBITMQ_AMQP_PORT`、`RABBITMQ_DEFAULT_USER`、
`RABBITMQ_DEFAULT_PASS` 和 `RABBITMQ_DEFAULT_VHOST` 覆盖。
...
Chroma 的 `resume_profile_chunks` collection。MySQL、RabbitMQ、Chroma 和切片参数
都可在 `.env` 中配置，完整示例见 `.env.example`。向量库只作为检索加速层，
写入失败不会撤销 MySQL 中已经确认的画像；前端会轮询状态并允许重试。

生产环境必须安装 `.[ocr]`，否则扫描型 PDF 在文本层不可用时会返回可读的 OCR 依赖错误。
同时请在 `.env` 中配置 `AI_SERVICE_API_KEY`、`DEEPSEEK_API_KEY` 和
`RESUME_ALLOWED_ORIGINS`。该配置必须与 Spring Boot 的 `MINIO_ENDPOINT` 完整来源一致，
